package brain 
{
	...
	public class Schedule extends Object
	{
		...
		/**
		 * Список условий для прерывания выполнения набора действий.
		 */
		public var interrupts:ConditionList; 
		...
		public function addFewInterrupts(interruptsList:Array /* of uint or int */):void
		{
			...
			for each (var i:int in interruptsList) 
			{
				interrupts.set(i);
			}
		...
		public function update():void
		{
			...
			if (o != null)
			{
				...
				{
					// Если функция вернула true, значит задача завершена, переходим к следующей задачи.
					_taskIndex++;
					if (_taskIndex >= tasks.length)
					{
						complete();// Все задачи выполнены.
					}
				}
			}
			else
